 ...
 *   id = "google_analytics_old_tac_service",
 *   label = "Google Analytics - old (Ga.js)",
 *   weight = 10
 * )
 */
class GoogleAnalyticsOldTacService implements TacServiceInterface {

  /**
   * Constant which stores the plugin ID.
   */
  const PLUGIN_ID = 'google_analytics_old_tac_service';

  /**
   * Constant which stores the Google analytics key field name.
   */
  const FIELD_GA_KEY = 'google_ga_old_key';

  /**
   * Constant which stores the library name.
   */
  const LIBRARY_NAME = 'tac_services/tac_google_ga_old';
